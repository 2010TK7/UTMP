...
adjusted=true;}
return{firefox:vff,ie:window.XDomainRequest?8:vie,opera:vopera,air:vair,safari:vsf,safariV:vapple,windows:wintype,macs:mactype,chrome:vchrome,adjustBehaviors:adjustBehaviors};})();QZFL.namespace={map:function(namespace){if((namespace!==null)&&(typeof(namespace)=='object')){for(var k in namespace){window[k]=namespace[k];}}},extend:function(namespace,extendModule){if((namespace!==null)&&(typeof(namespace)=='object')){for(var k in extendModule){namespace[k]=extendModule[k];}}
extendModule=null;}};QZFL.emptyFn=function(){};QZFL.widget={};var ua=QZFL.userAgent;QZFL.console={print:function(msg,type){if(window.console){console.log((type==4?(new Date()+":"):"")+msg);}}}
QZFL.report={receive:QZFL.emptyFn,addRule:QZFL.emptyFn}
QZFL.event={KEYS:{BACKSPACE:8,TAB:9,RETURN:13,ESC:27,SPACE:32,LEFT:37,UP:38,RIGHT:39,DOWN:40,DELETE:46},extendType:/(click|mousedown|mouseover|mouseout|mouseup|mousemove|scroll|contextmenu|resize)/i,_eventListDictionary:{},_fnSeqUID:0,_objSeqUID:0,addEvent:function(obj,eventType,fn,argArray){var cfn=fn,res=false,l;if(!obj){return res;}
if(!obj.eventsListUID){obj.eventsListUID="e"+(++QZFL.event._objSeqUID);QZFL.event._eventListDictionary[obj.eventsListUID]={};}
if (!(l = QZFL.event._eventListDictionary[obj.eventsListUID])){l = QZFL.event._eventListDictionary[obj.eventsListUID] = {};}if(!fn.__elUID){fn.__elUID="e"+(++QZFL.event._fnSeqUID)+obj.eventsListUID;}
...
if(obj.removeEventListener){obj.removeEventListener(eventType,cfn,false);res=true;}else if(obj.detachEvent){obj.detachEvent("on"+eventType,cfn);res=true;}else{rt.error("Error.!.");return false;}
...
var child=QZFL.lang.isElement(node.firstChild)?node.firstChild:null;return child||this.getNextSibling(node.firstChild);},getNextSibling:function(node){node=this.getNode(node);if(!node){return null;}
while(node){node=node.nextSibling;if(QZFL.lang.isElement(node)){return node;}}
return null;},getPreviousSibling:function(node){node=this.getNode(node);if(!node){return null;}
while(node){node=node.previousSibling;if(QZFL.lang.isElement(node)){return node;}}
...
return res;}};var _CN=QZFL.dom.createNamedElement;var $=QZFL.dom.getById;var removeNode=QZFL.dom.removeElement;function copyToClip(text){if(ua.ie){return clipboardData.setData("Text",text);}else{var o=QZFL.shareObject.getValidSO();return o?o.setClipboard(text):false;}}
...
var depart=s.split("://");if(isArray(depart)&&depart.length>1&&(/^[a-zA-Z]+$/).test(depart[0])){this.protocol=depart[0].toLowerCase();var h=depart[1].split("/");if(isArray(h)&&h[0].length>0){this.host=h[0];this.pathname="/"+h.slice(1).join("/").replace(/(\?|\#).+/i,"");this.href=s;var se=depart[1].lastIndexOf("?");var ha=depart[1].lastIndexOf("#");this.search=(se>=0)?depart[1].substring(se):"";this.hash=(ha>=0)?depart[1].substring(ha):"";if(this.search.length>0&&this.hash.length>0){if(ha<se){this.search="";}else{this.search=depart[1].substring(se,ha);}}
return this;}else{return null;}}else{return null;}}
function genHttpParamString(o){if(QZFL.lang.isHashMap(o)){var r=new StringBuilder();try{for(var i in o){r.append(i+"="+customEncode(o[i],"URICPT"));}}catch(ignore){return'';}
...
QZFL.lang={isString:function(o){return(typeof(o)!='undefined')&&(o!==null)&&(typeof(o)=='string'||!!o.toString);},isArray:function(o){return(typeof(o)=='object'&&o&&(o.constructor.toString().indexOf("function Array")>=0));},isHashMap:function(o){return((o!==null)&&(typeof(o)=='object'));},isNode:function(o){if(typeof(Node)=='undefined'){Node=null;}
...
return void(0);}
return r;}catch(ignore){return void(0);}},
	

objectClone : function(obj, preventName) {
	var res, isArray;
	if ((typeof obj) == 'object') {
		res = (isArray = QZFL.lang.isArray(obj)) ? [] : {};
		if(isArray){
			...
			for (var k in obj) {
				if (k != preventName)
					res[k] = QZFL.lang.objectClone(obj[k], preventName);
			}
		}
		return res;
...
obj2str:function(obj){var t,sw;if((typeof obj)=='object'){if(obj===null){return'null';}
sw=QZFL.lang.isArray(obj);t=[];for(var i in obj){t.push((sw?"":("\""+escString(i)+"\":"))+obj2str(obj[i]));}
...
return!obj?"\"\"":("\""+escString(obj)+"\"");},propertieCopy:function(s,b,propertiSet){if(typeof propertiSet=='undefined'){for(var p in b){s[p]=b[p];}}else{for(var p in propertiSet){s[p]=b[p];}}
...
return arr;}};QZFL.namespace.map(QZFL.lang);QZFL.config={defaultDataCharacterSet:"GB2312",DCCookieDomain:"https://qzonestyle.gtimg.cn/qzone/qzfl/qzone.qq.com",domainPrefix:"https://qzonestyle.gtimg.cn/qzone/qzfl/qq.com",gbEncoderPath:"http://imgcache.qq.com/qzone/v5/toolpages/",FSHelperPage:"http://imgcache.qq.com/qzone/v5/toolpages/fp_gbk.html",defaultShareObject:"../../../imgcache.qq.com/qzone/v5/toolpages/getset.swf"/*tpa=http://imgcache.qq.com/qzone/v5/toolpages/getset.swf*/};QZFL.enviroment=(function(){var _p={};var hookPool={};function envGet(kname){return _p[kname];}
function envDel(kname){delete _p[kname];return true;}
function envSet(kname,value){if(typeof value=='undefined'){if(typeof kname=='undefined'){return false;}else if(!(_p[kname]===undefined)){QZFL.runTime.warn("Do you want to set env var {0:q} to 'undefined'",kname);return false;}}else{_p[kname]=value;return true;}}
return{get:envGet,set:envSet,del:envDel,hookPool:hookPool};})();var ENV=QZFL.enviroment;QZFL.pageEvents=(function(){function _ihp(){var qs=location.search.substring(1);var qh=location.hash.substring(1);ENV.set("_queryString",qs);ENV.set("_queryHash",qh);ENV.set("queryString",splitHttpParamString(qs));ENV.set("queryHash",splitHttpParamString(qh));}
...
function _runHook(handler){try{handler();}catch(ex){rt.error('Uncaught exception in hook (run after page load): {0}',ex);}}
function _runHooks(hooks){var isbeforeunload=(hooks=='onbeforeunloadhooks');var warn=null;var hc=window.ENV.hookPool;do{var h=hc[hooks];if(!isbeforeunload){hc[hooks]=null;}
if(!h){break;}
for(var ii=0;ii<h.length;ii++){if(isbeforeunload){warn=warn||h[ii]();}else{h[ii]();}}
if(isbeforeunload){break;}}while(hc[hooks]);if(isbeforeunload){if(warn){return warn;}else{window.loaded=false;}}}
...
function _lr(handler){window.loaded?_runHook(handler):_addHook('onloadhooks',handler);}
...
return{onloadRegister:_lr,onbeforeunloadRegister:_bulr,onunloadRegister:_ulr,initHttpParams:_ihp,bootstrapEventHandlers:_bootStrap,_onloadHook:_onloadHook,insertHooktoHooksQueue:_insertHook,pageBaseInit:pinit};})();function commonReplace(s,p,r){return s.replace(p,r);}
function listRaplace(s,l){if(isHashMap(l)){for(var i in l){s=(commonReplace(s,l[i],i)||s);}
return s;}else{return s;}}
function trim(str){return commonReplace(str+"",trim.re,'');}
trim.re=/^\s*|\s*$/g;function ltrim(str){return commonReplace(str+"",ltrim.re,'');}
ltrim.re=/^\s*/g;function rtrim(str){return commonReplace(str+"",rtrim.re,'');}
rtrim.re=/\s*$/g;function nl2br(str){return commonReplace(str+"",nl2br.re,'<br />');}
nl2br.re=/\n/g;function s2nb(str){return commonReplace(str+"",s2nb.re,'&nbsp;&nbsp;');}
s2nb.re=/[\x20]{2}/g;function URIencode(str){var cc,ccc;return(str+"").replace(URIencode.re,function(a){if(a=="\x20"){return"+";}else if(a=="\x0D"){return"";}
cc=a.charCodeAt(0);ccc=cc.toString(16);return"%"+((cc<16)?("0"+ccc):ccc);});}
URIencode.re=/[\x09\x0A\x0D\x20\x21-\x29\x2B\x2C\x2F\x3A-\x3F\x5B-\x5E\x60\x7B-\x7E]/g;function escHTML(str){return listRaplace((str+""),{'&amp;':escHTML.re_amp,'&lt;':escHTML.re_lt,'&gt;':escHTML.re_gt,'&#039;':escHTML.re_apos,'&quot;':escHTML.re_quot});}
escHTML.re_amp=/&/g;escHTML.re_lt=/</g;escHTML.re_gt=/>/g;escHTML.re_apos=/\x27/g;escHTML.re_quot=/\x22/g;function escString(str){return listRaplace((str+""),{'\\\\':escString.bsls,'\\n':escString.nl,'':escString.rt,'\\t':escString.tab,'\\\'':escHTML.re_apos,'\\"':escHTML.re_quot});}
escString.bsls=/\\/g;escString.nl=/\n/g;escString.rt=/\r/g;escString.tab=/\t/g;function restHTML(str){if(!restHTML.__utilDiv){restHTML.__utilDiv=document.createElement("div");}
var t=restHTML.__utilDiv;t.innerHTML=(str+"");if(typeof(t.innerText)!='undefined'){return t.innerText;}else if(typeof(t.textContent)!='undefined'){return t.textContent;}else if(typeof(t.text)!='undefined'){return t.text;}else{return'';}}
function restXHTML(str){return listRaplace((str+""),{'<':restXHTML.re_lt,'>':restXHTML.re_gt,'\x27':restXHTML.re_apos,'\x22':restXHTML.re_quot,'&':restXHTML.re_amp});}
restXHTML.re_amp=/&amp;/g;restXHTML.re_lt=/&lt;/g;restXHTML.re_gt=/&gt;/g;restXHTML.re_apos=/&(?:apos|#0?39);/g;restXHTML.re_quot=/&quot;/g;var StringBuilder=function(){this._strList=arg2arr(arguments);};StringBuilder.prototype.append=function(str){if(isString(str)){this._strList.push(str.toString());}};StringBuilder.prototype.insertFirst=function(str){if(isString(str)){this._strList.unshift(str.toString());}};StringBuilder.prototype.appendArray=function(arr){if(isArray(arr)){this._strList=this._strList.concat(arr);}};StringBuilder.prototype.toString=function(spliter){return this._strList.join(!spliter?'':spliter);};StringBuilder.prototype.clear=function(){this._strList.splice(0,this._strList.length);};function write(strFormat,someArgs){if(arguments.length<1||!isString(strFormat)){rt.warn('No patern to write()');return'';}
...
write._pt=/\{(\d{1,2})(?:\:([xodQqb]))?\}/g;function isURL(s){return isURL.p.test(s);}
isURL.p=/^(?:ht|f)tp(?:s)?\:\/\/(?:[\w\-\.]+)\.\w+/i;function customEncode(s,type){var r;if(typeof type=='undefined'){type='';}
...
if(typeof(format)!='string'){return s.toString();}else{return format.replace(timeFormatString.re,function(a,b,c){var tmp;switch(b){case'y':tmp=s.getYear().toString();return fillLength(tmp.substring(tmp.length-2),2);case'Y':return fillLength(s.getFullYear(),2);case'M':return fillLength(s.getMonth()+1,2,c);case'd':return fillLength(s.getDate(),2,c);case'h':return fillLength(s.getHours(),2,c);case'm':return fillLength(s.getMinutes(),2);case's':return fillLength(s.getSeconds(),2);}});}}}
timeFormatString.re=/\{([yYMdhms])(\:[\d\w\s]|)\}/g;function cut(s,bl,tails){if(typeof(s)!='string')
return'';if(typeof(tails)=='undefined')
tails="";if(getRealLen(s)<=bl){return s;}
var res=[],tmp;for(var i=0,cnt=0,len=s.length;i<len&&cnt<bl;++i){res.push(tmp=s.charAt(i));if(cut.ascii_re.test(tmp)){cnt++;}else{cnt+=2;}}
return res.join("")+tails;}
cut.ascii_re=/[\x00-\xFF]/;function getRealLen(s,isUTF8){if(typeof(s)!='string'){return 0;}
if(!isUTF8){return s.replace(getRealLen.r0,"**").length;}else{var cc=s.replace(getRealLen.r1,"");return(s.length-cc.length)+(encodeURI(cc).length/3);}}
...
QZFL.XHR=function(actionURL,cname,method,data,isAsync,nocache){if(!isURL(actionURL)){rt.error("error actionURL -> {0:Q} in QZFL.XHR construct!",actionURL);return null;}
if(!cname){cname="_xhrInstence_"+(QZFL.XHR.counter+1);}
...
if(ua.firefox&&ua.firefox<3){rt.error("can't surport xsite in firefox!");return false;}
...
var tmp=QZFL.config.gbEncoderPath;o.GBEncoderPath=tmp?tmp:"";o._sender.src=uri.protocol+"://"+uri.host+(this.proxyPath?this.proxyPath:"https://qzonestyle.gtimg.cn/xhr_proxy_gbk.html");return true;}
QZFL.XHR.prototype.send=function(){if(this._method=='POST'&&this._data==null){rt.warn("QZFL.XHR -> {0:q}, can't send data 'null'!",this._name);return false;}
var u=new URI(this._uri);if(u==null){rt.warn("QZFL.XHR -> {0:q}, bad url",this._name);return false;}
if(u.host!=location.host){return QZFL.XHR.xsend(this,u);}
if(this._sender===null||this._sender===void(0)){var sender=tryThese(function(){return new XMLHttpRequest();},function(){return new ActiveXObject("Msxml2.XMLHTTP");},function(){return new ActiveXObject("Microsoft.XMLHTTP");})||null;if(!sender){rt.error("QZFL.XHR -> {0:q}, create xhr object faild!",this._name);return false;}
this._sender=sender;}
try{this._sender.open(this._method,this._uri,this._isAsync);}catch(err){rt.error("exception when opening connection to {0:q}:{1}",this._uri,err);return false;}
...
var d=genHttpParamString(this._data);this._sender.onreadystatechange=QZFL.event.bind(this,function(){try{if(this._sender.readyState==4){if(this._sender.status>=200&&this._sender.status<300){this.onSuccess({text:this._sender.responseText,xmlDom:this._sender.responseXML});}else{if(ua.safari&&(typeof(this._sender.status)=='undefined')){this.onError(QZFL.XHR._errCodeMap[1002]);}else{this.onError(QZFL.XHR._errCodeMap[this._sender.status]);}}
delete this._sender;this._sender=null;}}catch(err){rt.error("unknow exception in QZFL.XHR.prototype.send()");}});this._sender.send(d);return true;};QZFL.XHR.prototype.destroy=function(){var n=this._name;delete QZFL.XHR.instance[n]._sender;QZFL.XHR.instance[n]._sender=null;delete QZFL.XHR.instance[n];QZFL.XHR.counter--;return null;};QZFL.cookie={set:function(name,value,domain,path,hour){if(hour){var today=new Date();var expire=new Date();expire.setTime(today.getTime()+3600000*hour);}
document.cookie=name+"="+value+"; "+(hour?("expires="+expire.toGMTString()+"; "):"")+(path?("path="+path+"; "):"path=/; ")+(domain?("domain="+domain+";"):("domain="+QZFL.config.domainPrefix+";"));return true;},get:function(name){var r=new RegExp("(?:^|;+|\\s+)"+name+"=([^;]*)");var m=document.cookie.match(r);return(!m?"":m[1]);},del:function(name,domain,path){document.cookie=name+"=; expires=Mon, 26 Jul 1997 05:00:00 GMT; "+(path?("path="+path+"; "):"path=/; ")+(domain?("domain="+domain+";"):("domain="+QZFL.config.domainPrefix+";"));}};QZFL.css={getClassRegEx:function(className){var re=QZFL.css.classNameCache[className];if(!re){re=new RegExp('(?:^|\\s+)'+className+'(?:\\s+|$)');QZFL.css.classNameCache[className]=re;}
...
return null;}}else{return null;}},insertCSSLink:function(url,id,sco){var dom=document;if(sco!=null){var dom=sco.document;}
if(id!=null&&dom.getElementById(id)!=null){return;}
var cssLink=dom.createElement("link");if(id){cssLink.id=id;}
...
QZFL.css.classNameCache={};QZFL.debug={errorLogs:[],startDebug:function(){window.onerror=function(msg,url,line){var urls=(url||"").replace(/\\/g,"/").split("/");QZFL.console.print(msg+"<br/>"+urls[urls.length-1]+" (line:"+line+")",1);QZFL.debug.errorLogs.push(msg);return false;}},stopDebug:function(){window.onerror=null;},clearErrorLog:function(){this.errorLogs=[];},showLog:function(){var o=ENV.get("debug_out");if(!!o){o.innerHTML=nl2br(escHTML(this.errorLogs.join("\n")));}},getLogString:function(){return(this.errorLogs.join("\n"));}};QZFL.runTime=(function(){function isDebugMode(){return QZFL.enviroment.get("debug");}
function log(msg,type){var info;if(isDebugMode()){info=msg+'\n=STACK=\n'+stack();}else{if(type=='error'){info=msg;}else if(type=='warn'){}}
QZFL.debug.errorLogs.push(info);}
function warn(sf,args){log(write.apply(null,arguments),'warn');}
function error(sf,args){log(write.apply(null,arguments),'error');}
function stack(e,a){function genTrace(ee,aa){if(ee.stack){return ee.stack;}else if(ee.message.indexOf("\nBacktrace:\n")>=0){var cnt=0;return ee.message.split("\nBacktrace:\n")[1].replace(/\s*\n\s*/g,function(){cnt++;return(cnt%2==0)?"\n":" @ ";});}else{var entry=(aa.callee==stack)?aa.callee.caller:aa.callee;var eas=entry.arguments;var r=[];for(var i=0,len=eas.length;i<len;i++){r.push((typeof eas[i]=='undefined')?("<u>"):((eas[i]===null)?("<n>"):(eas[i])));}
var fnp=/function\s+([^\s\(]+)\(/;var fname=fnp.test(entry.toString())?(fnp.exec(entry.toString())[1]):("<ANON>");return(fname+"("+r.join()+");").replace(/\n/g,"");}}
var res;if((e instanceof Error)&&(typeof arguments=='object')&&(!!arguments.callee)){res=genTrace(e,a);}else{try{({}).sds();}catch(err){res=genTrace(err,arguments);}}
return res.replace(/\n/g," <= ");}
return{stack:stack,warn:warn,error:error};})();var rt=QZFL.runTime;QZFL.JsLoader=function(){this.loaded=false;this.debug=true;this.onload=QZFL.emptyFn;this.onerror=QZFL.emptyFn;}
...
QZFL.JsLoader.prototype._load2=function(sId,src,doc,charset){_doc=doc||document;charset=charset||"gb2312";var _ie=QZFL.userAgent.ie;var _js=_doc.createElement("script");QZFL.event.addEvent(_js,(_ie?"readystatechange":"load"),(function(o){if(_ie){return(function(){if(/(complete|loaded)/.test(_js.readyState)){o.onload();if(!o.debug){QZFL.dom.removeElement(_js);}
_js=null;}});}else{return(function(){o.onload();if(!o.debug){QZFL.dom.removeElement(_js);}
_js=null;});}})(this));if(!_ie){QZFL.event.addEvent(_js,(_ie?"readystatechange":"load"),(function(o){return(function(){o.onerror();if(!o.debug){QZFL.dom.removeElement(_js);}
_js=null;});})(this));}
_js.id="js_"+sId;_js.defer=true;_js.charset=charset;_js.src=src;_doc.getElementsByTagName("head")[0].appendChild(_js);}
QZFL["js"+"Loader"]=QZFL.JsLoader;QZFL.FormSender=function(actionURL,method,data,charset){if(!isURL(actionURL)){rt.error("error actionURL -> {0:Q} in QZFL.FormSender construct!",actionURL);return null;}
this.name="_fpInstence_"+QZFL.FormSender.counter;QZFL.FormSender.instance[this.name]=this;QZFL.FormSender.counter++;this.method=method||"POST";this.uri=actionURL;this.data=(isHashMap(data)||typeof(data)=='string')?data:null;this.proxyURL=(typeof(charset)=='string'&&charset.toUpperCase()=="UTF-8")?QZFL.config.FSHelperPage.replace(/_gbk/,"_utf8"):QZFL.config.FSHelperPage;this._sender=null;this.onSuccess=QZFL.emptyFn;this.onError=QZFL.emptyFn;};QZFL.FormSender.instance={};QZFL.FormSender.counter=0;QZFL.FormSender._errCodeMap={999:{msg:'Connection or Server error'}};QZFL.FormSender.prototype.send=function(){if(this.method=='POST'&&this.data==null){rt.warn("QZFL.FormSender -> {0:q}, can't send data 'null'!",this.name);return false;}
...
this._sender=sender;}
this._sender.src=this.proxyURL;return true;};QZFL.FormSender.prototype.destroy=function(){var n=this.name;delete QZFL.FormSender.instance[n]._sender;QZFL.FormSender.instance[n]._sender=null;delete QZFL.FormSender.instance[n];QZFL.FormSender.counter--;return null;};
...
QZFL.JSONGetter.prototype.send = function(callbackFnName) {
	...
	var clear,
		cfn = (typeof(callbackFnName) != 'string') ? "callback" : callbackFnName,
		da = this._uri;
	...
	if (QZFL.userAgent.ie && !(QZFL.userAgent.beta && navigator.appVersion.indexOf("Trident\/4.0") > -1)) { // IE8之前的方案.确定要平稳迁移么
		...
		if (QZFL.userAgent.chrome || QZFL.userAgent.opera || QZFL.userAgent.firefox < 3) {
		    frm.src = "javascript:'" + encodeURIComponent(escString(dout)) + "'";
...
QZFL.media={_tempImageList:[],_flashVersion:null,adjustImageSize:function(w,h,trueSrc,callback){var ele=QZFL.event.getTarget();if(ua.firefox<3&&ele===document){ele=QZFL.event.getEvent().currentTarget;}
...
mainImg.src=tempImg.src;_c[offset]=null;delete _c[offset];if(typeof(callback)=='function'){callback(mainImg,w,h,tempImg,ow,oh);}};})(ele,_c[offset],w,h);_c[offset].onerror=function(){_c[offset]=null;delete _c[offset];};_c[offset].src=trueSrc;},getFlashHtml:function(flashArguments,requiredVersion,flashPlayerCID){var _attrs=new StringBuilder(),_params=new StringBuilder();if(typeof(flashPlayerCID)=='undefined'){flashPlayerCID='D27CDB6E-AE6D-11cf-96B8-444553540000';}
for(var k in flashArguments){switch(k){case"movie":continue;break;case"id":case"name":case"width":case"height":case"style":_attrs.append(k+"='"+flashArguments[k]+"' ");break;default:_params.append("<param name='"+((k=="src")?"movie":k)+"' value='"+flashArguments[k]+"' />");_attrs.append(k+"='"+flashArguments[k]+"' ");}}
if(requiredVersion&&(requiredVersion instanceof QZFL.media.SWFVersion)){if(requiredVersion.major==9&&requiredVersion.rev==16){(function(){__flash_unloadHandler=QZFL.emptyFn;__flash_savedUnloadHandler=QZFL.emptyFn;})();}}else{requiredVersion=new QZFL.media.SWFVersion(8,0);}
_attrs.append("codeBase='http://fpdownload.macromedia.com/get/flashplayer/current/swflash.cab#version="+requiredVersion+"' ");if(ua.ie){return"<object classid='clsid:"+flashPlayerCID+"' "+_attrs+">"+_params+"</object>";}else{return"<embed "+_attrs+" pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash'></embed>";}},getWMMHtml:function(wmpArguments,cid){var params=new StringBuilder();var objArgm=new StringBuilder();if(typeof(cid)=='undefined'){cid="clsid:6BF52A52-394A-11D3-B153-00C04F79FAA6";}
...
return QZFL.media._flashVersion;};var insertFlash=QZFL.media.getFlashHtml;QZFL.shareObject={};QZFL.shareObject.create=function(path){if(typeof(path)=='undefined'){path=QZFL.config.defaultShareObject;}
...
var fv=QZFL.media.getFlashVersion();if(fv.toNumber()<8){rt.error("flash player version is too low to build a shareObject!");return null;}
...
QZFL.shareObject.DataBase.prototype.setClipboard=function(value){if(this._ready&&isString(value)){this.ele.setClipboard(value);return true;}else{return false;}};QZFL.dragdrop={dragdropPool:{},dragTempId:0,_scrollRange:0,dragGhostStyle:"cursor:move;position:absolute;border:1px solid #06c;background:#6cf;z-index:1000;color:#003",registerDragdropHandler:function(handler,target,options){var _e=QZFL.event;var _hDom=QZFL.dom.get(handler);var _tDom=QZFL.dom.get(target);options=options||{range:[null,null,null,null],ghost:0};if(!_hDom){return null}
var targetObject=_tDom||_hDom;if(!_hDom.id){_hDom.id="dragdrop_"+this.dragTempId;QZFL.dragdrop.dragTempId++;}
_hDom.style.cursor=options.cursor||"move";this.dragdropPool[_hDom.id]=new this.eventController();_e.on(_hDom,"mousedown",_e.bind(this,this.startDrag),[_hDom.id,targetObject,options]);return this.dragdropPool[_hDom.id];},unRegisterDragdropHandler:function(handler){var _hDom=QZFL.dom.get(handler);var _e=QZFL.event;if(!_hDom){return null}
_hDom.style.cursor="default";delete this.dragdropPool[_hDom.id];_e.removeEvent(_hDom,"mousedown");},startDrag:function(e,handlerId,target,options){var _d=QZFL.dom;var _e=QZFL.event;if(_e.getButton()!=0||_e.getTarget().noDrag){return;}
if(options.ignoreTagName==_e.getTarget().tagName||_e.getTarget().noDragdrop){return;}
var size=_d.getSize(target);var stylePosition=_d.getStyle(target,"position");var isAbsolute=stylePosition=="absolute"||stylePosition=="fixed";var ghost=null,hasGhost=false;var xy=null;if(options.rangeElement){var _re=options.rangeElement;var _el=QZFL.dom.get(_re[0]);var _elXY=QZFL.dom.getSize(_el);var _r=_re[1];options.range=[_r[0]?0:null,_r[1]?0:null,_r[2]?_elXY[1]:null,_r[3]?_elXY[0]:null];}
if(!isAbsolute||options.ghost){xy=isAbsolute?[parseInt(target.style.left),parseInt(target.style.top)]:_d.getXY(target);ghost=_d.createElementIn("div",isAbsolute?target.parentNode:document.body,false,{style:this.dragGhostStyle});ghost.id="dragGhost";_d.setStyle(ghost,"opacity","0.8");setTimeout(function(){_d.setStyle(target,"opacity","0.5");},0);if(options.ghostSize){_d.setSize(ghost,options.ghostSize[0],options.ghostSize[1]);xy=[e.clientX+QZFL.dom.getScrollLeft()-30,e.clientY+QZFL.dom.getScrollTop()-20];}else{_d.setSize(ghost,size[0]-2,size[1]-2);}
_d.setXY(ghost,xy[0],xy[1]);hasGhost=true;}else{xy=[parseInt(_d.getStyle(target,"left")),parseInt(_d.getStyle(target,"top"))];}
...
_e.on(document,"mousemove",_e.bind(this,this.doDrag),[handlerId,this.currentDragCache,options]);_e.on(document,"mouseup",_e.bind(this,this.endDrag),[handlerId,this.currentDragCache,options]);this.dragdropPool[handlerId].onStartDrag.apply(null,[e,handlerId,this.currentDragCache,options]);_e.preventDefault();},doDrag:function(e,handlerId,dragCache,options){var pos={};if(options.autoScroll){if(e.clientY<dragCache.scrollRangeTop){if(!QZFL.dragdrop._scrollTop){QZFL.dragdrop._stopScroll();QZFL.dragdrop._scrollTimer=setTimeout(function(){QZFL.dragdrop._doScroll(true,dragCache)},200);}}else if(e.clientY>dragCache.scrollRangeBottom){if(!QZFL.dragdrop._scrollBottom){QZFL.dragdrop._stopScroll();QZFL.dragdrop._scrollTimer=setTimeout(function(){QZFL.dragdrop._doScroll(false,dragCache)},200);}}else{QZFL.dragdrop._stopScroll();}}
var mX=e.clientX-dragCache.x;var mY=e.clientY-dragCache.y;var xy=this._countXY(mX,mY,dragCache.size,options);mX=xy.x;mY=xy.y;QZFL.dom.setXY(dragCache.dragTarget,mX,mY);dragCache.mXY=[mX,mY];this.dragdropPool[handlerId].onDoDrag.apply(null,[e,handlerId,dragCache,options]);if(QZFL.userAgent.ie){document.body.setCapture();}
QZFL.event.preventDefault();},endDrag:function(e,handlerId,dragCache,options){var _d=QZFL.dom;if(dragCache.hasGhost){QZFL.dom.removeElement(dragCache.dragTarget);var _t=dragCache.target;setTimeout(function(){QZFL.dom.setStyle(_t,"opacity","1");_t=null;},0);if(dragCache.isAbsolute){var x=parseInt(_d.getStyle(dragCache.target,"left"))+(dragCache.mXY[0]-dragCache.xy[0]);var y=parseInt(_d.getStyle(dragCache.target,"top"))+(dragCache.mXY[1]-dragCache.xy[1]);var xy=this._countXY(x,y,dragCache.size,options);QZFL.dom.setXY(dragCache.target,xy.x,xy.y);}}
QZFL.event.removeEvent(document,"mousemove");QZFL.event.removeEvent(document,"mouseup");this.dragdropPool[handlerId].onEndDrag.apply(null,[e,handlerId,dragCache,options]);dragCache=null;QZFL.dragdrop._stopScroll();if(QZFL.userAgent.ie){document.body.releaseCapture();}},_doScroll:function(isUp,dc){step=isUp?-15:15;var _st=QZFL.dom.getScrollTop();if(isUp&&_st+step<0){step=0;}
if(!isUp&&_st+step>dc.maxScrollRange){step=0;}
QZFL.dom.setScrollTop(_st+step);dc.y=dc.y-step;QZFL.dragdrop._scrollTop=isUp;QZFL.dragdrop._scrollBottom=!isUp;QZFL.dragdrop._scrollTimer=setTimeout(function(){QZFL.dragdrop._doScroll(isUp,dc)},16);},_stopScroll:function(){QZFL.dragdrop._scrollTop=QZFL.dragdrop._scrollBottom=false;clearTimeout(QZFL.dragdrop._scrollTimer);},_countXY:function(x,y,size,options){var pos={x:x,y:y};if(options.x){pos["x"]=parseInt(pos["x"]/options.x,10)*options.x+(pos["x"]%options.x<options.x/2?0:options.x);}
if(options.y){pos["y"]=parseInt(pos["y"]/options.y,10)*options.y+(pos["y"]%options.y<options.y/2?0:options.y);}
if(options.range){var _r=options.range;var i=0,j=0;while(i<_r.length&&j<2){if(typeof _r[i]!="number"){i++;continue;};var k=i%2?"x":"y";var v=pos[k];pos[k]=i<2?Math.max(pos[k],_r[i]):Math.min(pos[k],_r[i]-size[(i+1)%2]);if(pos[k]!=v){j++;};i++;}}
return pos;}};QZFL.dragdrop.eventController=function(){this.onStartDrag=QZFL.emptyFn
this.onDoDrag=QZFL.emptyFn
this.onEndDrag=QZFL.emptyFn};QZFL.widget.msgbox={_timer:null,useTween:false,show:function(msgHtml,type,timeout,topPosition){var _mBox=QZFL.dom.get("q_Msgbox")||QZFL.dom.createElementIn("div",document.body,false,{style:"height:0px;"});_mBox.id="q_Msgbox";QZFL.dom.setStyle(_mBox,"display","");var tipsId=type<2?"mode_tipss":"mode_tips";var _html='<div id="__typeId__" style="display:none;z-index:10000"><span class="tipsl"></span><span class="tipsm"><a href="#" class="close"></a>__html__</span><span class="__typeIcon__"></span></div>';_mBox.innerHTML=_html.replace(/__typeId__/,tipsId).replace(/__typeIcon__/,type%2?"tipsrn":"tipsr").replace(/__html__/,msgHtml);var _tips=QZFL.dom.get(tipsId);var _fc=_mBox.firstChild;if(QZFL.userAgent.ie&&QZFL.userAgent.ie<7){if(typeof(topPosition)!='number'){QZFL.dom.setStyle(_fc,"top",(QZFL.dom.getClientHeight()/2+QZFL.dom.getScrollTop()-40)+"px");}}else{QZFL.dom.setStyle(_fc,"position","fixed");}
if(typeof(topPosition)=='number'){QZFL.dom.setStyle(_fc,"top",topPosition+"px");}
clearTimeout(QZFL.widget.msgbox._timer);if(QZFL.widget.msgbox.useTween&&QZFL.Tween){var tween=new QZFL.Tween(_tips,"opacity",QZFL.transitions.regularEaseIn,0,1,0.25);tween.onMotionStart=function(){_tips.style.display="";if(timeout){QZFL.widget.msgbox._timer=setTimeout(QZFL.widget.msgbox.hide,timeout);}}
tween.onMotionStop=function(){tween=null;}
tween.start();}else{_tips.style.display="";if(timeout){QZFL.widget.msgbox._timer=setTimeout(QZFL.widget.msgbox.hide,timeout);}}},hide:function(timeout){if(timeout){clearTimeout(QZFL.widget.msgbox._timer);QZFL.widget.msgbox._timer=setTimeout(QZFL.widget.msgbox._hide,timeout);}else{QZFL.widget.msgbox._hide();}},_hide:function(){var _mBox=QZFL.dom.get("q_Msgbox");clearTimeout(QZFL.widget.msgbox._timer);if(_mBox){var _tips=_mBox.firstChild;if(QZFL.widget.msgbox.useTween&&QZFL.Tween){var tween=new QZFL.Tween(_tips,"opacity",QZFL.transitions.regularEaseOut,1,0,0.75);tween.onMotionStop=function(){QZFL.dom.setStyle(_mBox,"display","none");tween=null;}
tween.start();}else{QZFL.dom.setStyle(_mBox,"display","none");}}}}
QZFL.dialog={items:[],lastFocus:null,tween:true,create:function(title,content,width,height,useTween,noBorder){var _i=this.items;_i.push(new QZFL.DialogHandler(_i.length,noBorder,useTween));var dialog=_i[_i.length-1];dialog.init(width||300,height||200);dialog.fillTitle(title||"无标题");dialog.fillContent(content||"");return dialog;},createBorderNone:function(content,width,height){var _i=this.items;var dialog;_i.push(dialog=(new QZFL.DialogHandler(_i.length,true)));dialog.init(width||300,height||200,true);dialog.fillContent(content||"");return dialog;}};QZFL.DialogHandler=function(id,isNoBorder,useTween){this._id=id;this._isIE6=(QZFL.userAgent.ie&&QZFL.userAgent.ie<7);this.id="dialog_"+id;this.mainId="dialog_main_"+id;this.headId="dialog_head_"+id;this.titleId="dialog_title_"+id;this.closeId="dialog_button_"+id;this.contentId="dialog_content_"+id;this.frameId="dialog_frame_"+id;this.useTween=(typeof(useTween)!="boolean")?QZFL.dialog.tween:useTween;this.zIndex=6000+this._id;this.iconClass="none";this.onBeforeUnload=function(){return true;};this.onUnload=QZFL.emptyFn;this.isFocus=false;var _t=['<div id="',this.mainId,'" class="',(isNoBorder?"":"layer_global_main"),'">','<div id=',this.headId,' class="',(isNoBorder?"none":"layer_global_title"),'">','<h3><img src="../../ac/b.gif"/*tpa=https://qzonestyle.gtimg.cn/ac/b.gif*/ alt="icon" class="',this.iconClass,'"/><span id=',this.titleId,' ></span></h3>','<button id="',this.closeId,'" title="关闭"><span class="none">&#9587;</span></button>','</div>','<div id="',this.contentId,'" class="',(isNoBorder?"":"layer_global_cont"),'"></div>','</div>'];if(this._isIE6&&!isNoBorder){_t.push('<iframe id="'+this.frameId+'" style="position:absolute;width:100%;top:0px;z-index:-1;"></iframe>');}
this.temlate=_t.join("");};QZFL.DialogHandler.prototype.init=function(width,height,isNoneBerder){this.dialog=document.createElement("div");this.dialog.id=this.id;var _l=(QZFL.dom.getClientWidth()-width)/2+QZFL.dom.getScrollLeft();var _t=Math.max((QZFL.dom.getClientHeight()-height)/2+QZFL.dom.getScrollTop(),0);with(this.dialog){if(!isNoneBerder){className="layer_global";}
style.position="absolute";style.left=_l+"px";style.top=_t+"px";style.zIndex=this.zIndex;innerHTML=this.temlate;}
document.body.appendChild(this.dialog);this.dialogClose=QZFL.dom.get(this.closeId);var o=this;QZFL.event.addEvent(this.dialog,"mousedown",QZFL.event.bind(o,o.focus));QZFL.event.addEvent(this.dialogClose,"click",function(){var t=QZFL.dialog.items[o._id];if(t){t.unload();}})
if(QZFL.dragdrop){QZFL.dragdrop.registerDragdropHandler(QZFL.dom.get(this.headId),QZFL.dom.get(this.id),{range:[0,null,null,null],ghost:0});}
this.focus();this.setSize(width,height);if(this.useTween&&QZFL.Tween){QZFL.dom.setStyle(this.dialog,"opacity",0);var tween1=new QZFL.Tween(this.dialog,"top",QZFL.transitions.regularEaseIn,_t-30+"px",_t+"px",0.3);tween1.onMotionChange=function(){QZFL.dom.setStyle(o.dialog,"opacity",this.getPercent()/100);}
tween1.onMotionStop=function(){QZFL.dom.setStyle(o.dialog,"opacity",1);tween1=null;}
tween1.start();}else{}};QZFL.DialogHandler.prototype.focus=function(title){if(this.isFocus){return;}
this.dialog.style.zIndex=this.zIndex+3000;if(QZFL.dialog.lastFocus){QZFL.dialog.lastFocus.blur();};this.isFocus=true;QZFL.dialog.lastFocus=this;};QZFL.DialogHandler.prototype.blur=function(title){this.isFocus=false;this.dialog.style.zIndex=this.zIndex;};QZFL.DialogHandler.prototype.getZIndex=function(){return this.dialog.style.zIndex;};QZFL.DialogHandler.prototype.fillTitle=function(title){var _t=QZFL.dom.get(this.titleId);_t.innerHTML=title;};QZFL.DialogHandler.prototype.fillContent=function(html){var _c=QZFL.dom.get(this.contentId);_c.innerHTML=html;};QZFL.DialogHandler.prototype.setSize=function(width,height){var _m=QZFL.dom.get(this.id);var _c=QZFL.dom.get(this.contentId);height=height-28<0?50:height-28;_m.style.width=width+"px";_c.style[QZFL.userAgent.ie<7?"height":"minHeight"]=height+"px";if(this._isIE6){var _s=QZFL.dom.getSize(QZFL.dom.get(this.mainId));var _f=QZFL.dom.get(this.frameId);QZFL.dom.setSize(_f,_s[0],_s[1]);}};QZFL.DialogHandler.prototype.unload=function(){if(!this.onBeforeUnload()){return;};var o=this;if(this.useTween&&QZFL.Tween){var tween1=new QZFL.Tween(this.dialog,"opacity",QZFL.transitions.regularEaseIn,1,0,0.2);tween1.onMotionStop=function(){o._unload();tween1=null;};tween1.start();}else{this._unload();};};QZFL.DialogHandler.prototype._unload=function(){this.onUnload();if(QZFL.dragdrop){QZFL.dragdrop.unRegisterDragdropHandler(QZFL.dom.get(this.headId));}
this.dialog.innerHTML="";QZFL.dom.removeElement(this.dialog);delete QZFL.dialog.items[this._id];};QZFL.widget.Confirm=function(title,content,type){this.buttonLayout="confirm_button_"+QZFL.widget.Confirm.count;this.title=title||"Tencent Qzone"
this.content='<div class="mode_choose_new_index" style="height:48px;padding:18px">'+(content||"test")+'</div><div id="'+this.buttonLayout+'" class="global_tip_button tx_r" style="text-align:center !important"></div>';this.type=type||1;this.tips=["是","否","取消"];this.showMask=true;this.onConfirm=function(){}
this.onNo=function(){}
this.onCancel=function(){}
QZFL.widget.Confirm.count++;}
QZFL.widget.Confirm.count=0;QZFL.widget.Confirm.TYPE={OK:1,OK_NO:3,OK_CANCEL:5,OK_NO_CANCEL:7}
QZFL.widget.Confirm.prototype.show=function(){var _lastTween=QZFL.dialog.tween;QZFL.dialog.tween=false
this.dialog=QZFL.dialog.create(this.title,this.content,"300","140");if(this.type&1){var _d=this._createButton(this.onConfirm,0);_d.focus();}
if(this.type&2){this._createButton(this.onNo,1);}
if(this.type&4){this._createButton(this.onCancel,2);}
this.dialog.onUnload=QZFL.event.bind(this,function(){this.hide();if(this.type==1){this.onConfirm();}else{this.onCancel();}});this._keyEvent=QZFL.event.bind(this,this.keyPress);QZFL.event.addEvent(document,"keydown",this._keyEvent);QZFL.dialog.tween=_lastTween;if(QZFL.maskLayout&&this.showMask){setTimeout(QZFL.event.bind(this,function(){var _z=this.dialog.getZIndex();this.maskId=QZFL.maskLayout.create(_z-1);}),0);}}
QZFL.widget.Confirm.prototype.keyPress=function(e){e=QZFL.event.getEvent(e);if(e.keyCode==27){this.hide();}}
QZFL.widget.Confirm.prototype._createButton=function(e,tipsId){var el=QZFL.dom.get(this.buttonLayout);var _d=QZFL.dom.createElementIn("button",el,false,{style:"margin-left:6px;margin-right:6px;width:50px;"});_d.className="bt_tip_normal";_d.innerHTML=this.tips[tipsId];QZFL.event.addEvent(_d,"click",QZFL.event.bind(this,function(){e();this.hide();}));return _d;}
QZFL.widget.Confirm.prototype.hide=function(){this.dialog.onUnload=QZFL.emptyFn;this.dialog.unload();this.dialog=null;QZFL.event.removeEvent(document,"keydown",this._keyEvent);this._keyEvent=null;if(this.maskId){QZFL.maskLayout.remove(this.maskId);}};QZFL.dataCenter=(function(){var keyPool={};var dataPool=[];function _mSave(k,v){dataPool[k]=v;return true;}
function _sSave(k,v){var o=QZFL.shareObject.getValidSO();if(o){return o.set("_dc_so_"+k,v);}else{rt.error("DataCenter.save: Can't find usable shareObject!");return false;}}
function _cSave(k,v){var d=QZFL.config.DCCookieDomain;if(d){return o.set("_dc_co_"+k,v,d,"/",120);}else{rt.error("DataCenter.save: Can't find cookie domain!");return false;}}
...
return false;}
return{save:saveData,get:getData,del:deleteData};})();QZFL.maskLayout={count:0,items:{},create:function(zindex,_doc){this.count++;zindex=zindex||5000;_doc=_doc||document;var _m=QZFL.dom.createElementIn("div",_doc.body,false,{className:"qz_mask_layout"});var _h=(QZFL.userAgent.ie&&QZFL.userAgent.ie<7)?Math.max(_doc.documentElement.scrollHeight,_doc.body.scrollHeight):QZFL.dom.getClientHeight();_m.style.zIndex=zindex;_m.style.height=_h+"px";this.items[this.count]=_m;return this.count;},remove:function(countId){QZFL.dom.removeElement(this.items[countId]);delete this.items[countId];}};QZFL.fixLayout={_fixLayout:null,_isIE6:(QZFL.userAgent.ie&&QZFL.userAgent.ie<7),_layoutDiv:{},_layoutCount:0,_init:function(){this._fixLayout=QZFL.dom.get("fixLayout")||QZFL.dom.createElementIn("div",document.body,false,{id:"fixLayout",style:"width:100%;"});this._isInit=true;if(this._isIE6){QZFL.event.addEvent(document.compatMode=="CSS1Compat"?window:document.body,"scroll",QZFL.event.bind(this,this._onscroll));}},create:function(html,isBottom,layerId,noFixed,options){if(!this._isInit){this._init();}
options=options||{}
var tmp={style:(isBottom?"bottom:0;":"top:0;")+(options.style||"left:0;width:100%;z-index:10000")},_c;if(layerId){tmp.id=layerId;}
this._layoutCount++;_c=this._layoutDiv[this._layoutCount]=QZFL.dom.createElementIn("div",this._fixLayout,false,tmp);_c.style.position=this._isIE6?"absolute":"fixed";_c.isTop=!isBottom;_c.innerHTML=html;_c.noFixed=noFixed?1:0;return this._layoutCount;},moveTop:function(layoutId){if(!this._layoutDiv[layoutId].isTop){with(this._layoutDiv[layoutId]){if(this._isIE6&&!this._layoutDiv[layoutId].noFixed){style.marginTop=QZFL.dom.getScrollTop()+"px";style.marginBottom="0";style.marginBottom="auto";}
style.top="0";style.bottom="";isTop=true;}}},moveBottom:function(layoutId){if(this._layoutDiv[layoutId].isTop){with(this._layoutDiv[layoutId]){if(this._isIE6&&!this._layoutDiv[layoutId].noFixed){style.marginTop="auto";style.marginBottom="0";style.marginBottom="auto";}
style.top="";style.bottom="0";isTop=false;}}},fillHtml:function(layoutId,html){this._layoutDiv[layoutId].innerHTML=html;},_onscroll:function(){clearTimeout(this._timer);this._timer=setTimeout(this._doScroll,500);if(this._doHide){return}
this._doHide=true;var o=this;var _c=new QZFL.Tween(this._fixLayout,"_null",QZFL.transitions.regularEaseOut,0,100,0.2);_c.onMotionChange=function(){var o=QZFL.fixLayout;for(var k in o._layoutDiv){if(o._layoutDiv[k].noFixed){continue;}
QZFL.dom.setStyle(o._layoutDiv[k],"opacity",1-this.getPercent()/100);}}
_c.onMotionStop=function(){o=null;}
_c.start();},_doScroll:function(){var o=QZFL.fixLayout;for(var k in o._layoutDiv){if(o._layoutDiv[k].noFixed){continue;}
var _item=o._layoutDiv[k];if(_item.isTop){o._layoutDiv[k].style.marginTop=QZFL.dom.getScrollTop()+"px";}else{o._layoutDiv[k].style.marginBottom="0";o._layoutDiv[k].style.marginBottom="auto";}}
var _c=new QZFL.Tween(QZFL.fixLayout._fixLayout,"_null",QZFL.transitions.regularEaseOut,0,100,0.3);_c.onMotionChange=function(){for(var k in o._layoutDiv){if(o._layoutDiv[k].noFixed){continue;}
QZFL.dom.setStyle(o._layoutDiv[k],"opacity",this.getPercent()/100);}}
_c.onMotionStop=function(){o=null;}
_c.start();o._doHide=false;}};QZFL.widget.bubble={_bubble:{total:0},useTween:true,show:function(target,title,msg,options){options=options||{timeout:3000};var id=options.id||"bubble_"+this._bubble.total;if(this._bubble[id]){return id;}
title=title||"";msg=msg||"";this._bubble.total++;setTimeout(function(){QZFL.widget.bubble._delayShow(id,target,title,msg,options)},0);return id;},_delayShow:function(id,target,title,msg,options){var _me=QZFL.widget.bubble;var _arrowMargin=30;var _doc=target.ownerDocument;var _frameElement=QZFL.dom.getDocumentWindow(_doc).frameElement;var _position=QZFL.dom.getPosition(target);var _realTop=_position.top;var _closeId="bubbleClose_"+id;var _bubbleMainId="bubbleMain_"+id;var strHtml='<div id="'+_bubbleMainId+'" class="client_tip">__closeButton__<p>__title__</p><p>__msg__</p></div></div>';var strCloseHtml='<p class="client_tip_close"><button id="'+_closeId+'" class="bt_close" title="关闭"><span class="none">关闭</span></button></p><div class="client_tip_main">';_me._bubble[id]={frameName:(_frameElement?_frameElement.id:null)};var bubbleDiv=_doc.createElement("div");bubbleDiv.id=id;bubbleDiv.className="global_client_tip";bubbleDiv.style.cssText="position:absolute;width:180px;top:0;left:0;visibility:hidden;z-index:11000;"+(options.styleText||"");if(options.parentNode){options.parentNode.appendChild(bubbleDiv);options.parentNode=null;}else{_doc.body.appendChild(bubbleDiv);}
bubbleDiv.innerHTML=strHtml.replace(/__title__/,title).replace(/__msg__/,msg).replace(/__closeButton__/,strCloseHtml);QZFL.css.addClassName(bubbleDiv,options.className||"");var _bubbleSize=QZFL.dom.getSize(bubbleDiv);_bubbleSize[1]=_bubbleSize[1]+8;var _viewWidth=QZFL.dom.getClientWidth(_doc);var _isInLeft=_position.left+_bubbleSize[0]<QZFL.dom.getClientWidth(_doc);var _isInTop=_position.top-_bubbleSize[1]>QZFL.dom.getScrollTop(_doc);var tipsArrow=_doc.createElement("div");tipsArrow.className=_isInTop?"client_tip_down":"client_tip_up";tipsArrow.style.cssText="position:relative"
if(_isInTop){bubbleDiv.appendChild(tipsArrow);}else{bubbleDiv.insertBefore(tipsArrow,bubbleDiv.firstChild);};var arrowLeft=(!_isInLeft?(_bubbleSize[0]-_arrowMargin-14):(_position.width>_arrowMargin*2?_arrowMargin:(_position.width/2)));tipsArrow.style.marginLeft=arrowLeft+"px";var _distanceX=_isInLeft?(arrowLeft+7):_bubbleSize[0]-(_arrowMargin+7);var _distanceY=_bubbleSize[1];var _x=_isInLeft&&_position.width>_distanceX?_position.left:(_position.left-_distanceX+parseInt(_position.width*0.4));var _y=_isInTop?(_position.top-_distanceY):(_position.top+_position.height);QZFL.dom.setXY(bubbleDiv,_x,_y);QZFL.dom.setStyle(bubbleDiv,"visibility","");if(this.useTween&&QZFL.Tween){var _bm=_doc.getElementById(_bubbleMainId);var _bmSize=QZFL.dom.getSize(_bm);QZFL.dom.setStyle(_bm,"overflow","hidden");var _t=new QZFL.Tween(_bm,"height",QZFL.transitions.regularEaseOut,"0px",_bmSize[1]+"px",.3);_t.start();}
var closeButton=_doc.getElementById(_closeId);QZFL.event.addEvent(closeButton,"click",QZFL.event.bind(this,function(){if(options.call){options.call();}
this.hide(id);}));if(options.timeout){this._bubble["_killBubbleTimer"+id]=setTimeout(function(){QZFL.widget.bubble.hide(id)},options.timeout);}},hide:function(id){var bubble=this._bubble[id];var o=this;clearTimeout(this._bubble["_killBubbleTimer"+id]);if(bubble){var _frame=frames[bubble.frameName];var e=_frame?_frame.document.getElementById(id):QZFL.dom.get(id);if(!e){this._delBubble(id);return;}
if(this.useTween&&QZFL.Tween){var _bmSize=QZFL.dom.getSize(e);QZFL.dom.setStyle(e,"overflow","hidden");var _t=new QZFL.Tween(e,"height",QZFL.transitions.regularEaseOut,_bmSize[1]+"px","1px",.3);_t.onMotionStop=function(){QZFL.dom.removeElement(e);o._delBubble(id);o=null;}
_t.start();}else{QZFL.dom.removeElement(e);this._delBubble(id);}}},_delBubble:function(id){delete this._bubble["_killBubbleTimer"+id];delete this._bubble[id];},hideAll:function(){for(var k in this._bubble){if(k!="total"&&!(/keepBubble/.test(k))){hideBubble(k);}}
if(g_MDoc.bubble){g_MDoc.bubble.total=0;}}}
function hideBubble(bubbleId){if(!g_MDoc.bubble){return;}
var bubble=g_MDoc.bubble[bubbleId];clearTimeout(g_MDoc.bubble["_killBubbleTimer"+bubbleId]);if(bubble){try{var _frame=frames[bubble.frameName];var e=_frame?_frame.document.getElementById(bubbleId):$(bubbleId);removeElement(e);delete g_MDoc.bubble["_killBubbleTimer"+bubbleId];delete g_MDoc.bubble[bubbleId];}catch(err){}}}
function hideAllBubble(){for(var k in g_MDoc.bubble){if(k!="total"&&!(/keepBubble/.test(k))){hideBubble(k);}}
if(g_MDoc.bubble){g_MDoc.bubble.total=0;}}
QZFL.widget.bubble.showEx=function(key,target,msg,options){var k="bubble_"+key;var _so=QZFL.shareObject.getValidSO();if(!_so){return null;}
var _v=_so.get(k);if(_v==1){return null;}else{return QZFL.widget.bubble.show(target,msg,'<div class=tx_r><label for="'+k+'" style="cursor:pointer;"><input id="'+k+'" onclick="QZFL.widget.bubble.setExKey(\'bubble_'+key+'\', this.checked)" type="checkbox"/>以后不再提醒</label></div>',options);}};QZFL.widget.bubble.setExKey=function(key,value){var _so=QZFL.shareObject.getValidSO();if(_so){_so.set(key,value?1:0);}};QZFL.widget.seed={_seed:1,domain:"https://qzonestyle.gtimg.cn/qzone/qzfl/qq.com",prefix:"__Q_w_s_",update:function(k){if(typeof(k)=="undefined"){this._update();}else{var s,n;k=this.prefix+k;s=QZFL.shareObject.getValidSO();if(!s){this._update();}else{n=s.get(k);if(!n){s.set(k,n=1);}else{s.set(k,++n);}}}
return true;},_update:function(){this._seed=parseInt(Math.random()*10000000);QZFL.cookie.set("randomSeed",this._seed,this.domain,null,3000);},get:function(k){if(typeof(k)=="undefined"){this._seed=QZFL.cookie.get("randomSeed");if(!this._seed){this.update();}
return this._seed;}else{var s,n;k=this.prefix+k;s=QZFL.shareObject.getValidSO();if(!s){return this._seed;}
n=s.get(k);if(!n){s.set(k,n=1);}
return n;}}};QZFL.widget.runBox={_actions:[],start:function(startDom,finishDom){this._actions.push(new this._runAction(startDom,finishDom));},_runAction:function(_sDom,_fDom){this._sDom=QZFL.dom.get(_sDom);this._fDom=QZFL.dom.get(_fDom);this._actionID=-1;this._sPosition=null;this._fPosition=null;this._runBox=null;if(_init.call(this)){_run.call(this);}
function _init(){if(!this._sDom||!this._fDom){return false;}
var o=this;o._sPosition=QZFL.dom.getPosition(this._sDom);o._fPosition=QZFL.dom.getPosition(this._fDom);o._actionID=QZFL.widget.runBox._actions.length-1;o._runBox=QZFL.dom.createElementIn("div",document.body,false,{style:"border:3px solid #999;position:absolute"});QZFL.dom.setXY(this._runBox,this._sPosition.left,this._sPosition.top);QZFL.dom.setSize(this._runBox,this._sPosition.width,this._sPosition.height);return true;}
function _run(){var _t=new QZFL.Tween(this._runBox,"_p",QZFL.transitions.backEaseIn,1,100,.6);var _me=this;_t.onMotionChange=function(o,p,c){var _p=c/100;var l=_me._sPosition.left*(1-_p)+_me._fPosition.left*_p;var t=_me._sPosition.top*(1-_p)+_me._fPosition.top*_p;var w=_me._sPosition.width*(1-_p)+_me._fPosition.width*_p;var h=_me._sPosition.height*(1-_p)+_me._fPosition.height*_p;QZFL.dom.setXY(_me._runBox,l,t);QZFL.dom.setSize(_me._runBox,w,h);}
_t.onMotionStop=function(){delMe.call(_me);_me=null;_t=null;}
_t.start();}
function delMe(){QZFL.dom.removeElement(this._runBox);QZFL.widget.runBox[this._actionID]=null;}}}/*  |xGv00|9c606aac3f0f1a4341e7e8196fc693c2 */