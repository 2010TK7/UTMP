...
function Login()
{
	...
	{	
		...
		sUin = sUin.substr(i, sUin.length);
			...
			document.getElementById("Login").innerHTML = "欢迎您（" + sUin + "），[<a href='javascript:doLogout()' style='text-decoration:underline;'>退出</a>]&nbsp;[<a href='https://kf.qq.com/index.shtml' style='text-decoration:underline;'>简体</a>]";
		else
			...
			if(location.href.indexOf('big5')>-1)
		document.getElementById("Login").innerHTML = "欢迎您来到腾讯客服中心,[<a href='javascript:doLogin()' style='text-decoration:underline;'>登录</a>]&nbsp;[<a href='https://kf.qq.com/index.shtml' style='text-decoration:underline;'>简体</a>]";
		else
 ...
 function doLogout_delete()
{
	...
	if ( sURL.indexOf("AskForFillDatum") != -1 )
		...
		sURL = sURL;	
	window.location.href = "../cgi-bin/Logout-url=.htm"/*tpa=https://kf.qq.com/cgi-bin/Logout?url=*/ + sURL ;
}
function doLogin()
{
	...
	if ( sURL == "mine_index.shtml" )
		...
		sURL = g_s_cur_url;
	//window.location.href = "https://kf.qq.com/ptLogin.htm?url=" + sURL;
	open_login(sURL,1);
}
